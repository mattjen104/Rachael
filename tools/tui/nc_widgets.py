...
def detect_media_capabilities():
    caps = {"sixel": False, "kitty": False, "iterm2": False}
    ...
    try:
        import subprocess
        result = subprocess.run(["tput", "Smulx"], capture_output=True, timeout=1)
...
class WidgetManager:
    ...
    def create_multiselector(self, items: list, title: str = "Select"):
        ...
        try:
            ...
            self.selector = NcMultiSelector.create(
                self.selector_plane, nc_items, title=title, maxdisplay=sel_h - 3)
            return self.selector
        except Exception as e:
    ...
    def create_sparkline(self, parent_plane, rows_h: int, cols_w: int, y: int, x: int):
        ...
        if plot_cls is None:
            ...
            return None
        try:
            self.sparkline_plane = NcPlane(parent_plane, rows_h, cols_w, y, x)
            opts = {}
            blit = NCBLIT_BRAILLE or NCBLIT_4x2 or NCBLIT_2x2
            if blit is not None:
                opts["gridtype"] = blit
            self.sparkline = plot_cls.create(self.sparkline_plane, **opts)
            return self.sparkline
        except Exception as e:
            self.log_degradation("NcPlot/NcDPlot", str(e))
            return None

    def add_sparkline_sample(self, value: float):
        if self.sparkline:
            try:

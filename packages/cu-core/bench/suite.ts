...
import type { ObservationKind, SurfaceKind } from "../src/types";

// ---------------------------------------------------------------------------
// 30-task curated suite — 10 web, 10 Windows-UIA, 10 Citrix-vision.
//
// These are derived from real Rachael trajectories; the actual recordings
// live under tests/fixtures/trajectories/ and are loaded by the OSS-extraction
// task. Here we ship the *spec* shape (intent + step count + expected tier)
// so the bench harness can be exercised end-to-end with FakeSurface today.
//
// Each task carries a `notes` string capturing what the production trajectory
// did so a contributor can reconstruct it without reading the JSON dump.
// ---------------------------------------------------------------------------
...
export const SUITE_ENTRIES: SuiteEntry[] = [
  // ---- Web (browser-tab / browser-extension) ----
  ...
  entry("uia-10-uia-fallback-coords", "desktop-window", "click custom WPF control with no UIA name", "RawScreenshot", 2, "UIA misses → fallback to image → coord click (counts as miss)"),

  // ---- Citrix vision (citrix-session) ----
...
// Baseline numbers locked in for the harness so a regression in tier mix or
// coord-click rate is visible at PR time. These come from running the suite
// against a stubbed surface that mimics production tier-hit rates from
// October 2025 telemetry. Re-run with `tsx packages/cu-core/bench/run.ts`
// and update `bench/baseline.json` deliberately.
...
time. Numbers in baseline.json are the lock-in baseline.`;

import type { Surface } from "../src/bus";
import type { Action, ObservationKind, SurfaceKind, Verifier } from "../src/types";
import { Budget, InMemoryTraceSink, Router, type RouterStepInput } from "../src/router/index";

// ---------------------------------------------------------------------------
// Bench harness — task spec → run → score → report.
//
// Intentionally small. The OSS-extraction task will swap in OSWorld /
// WebArena scale; this is the *shape* the per-task suite needs to fit.
// Every run stays in-process; surfaces are constructed by a factory the
// caller supplies (real adapters in CI, FakeSurface in unit tests).
// ---------------------------------------------------------------------------
...
export interface TaskSpec {
  ...
  // What we expect at the end. Bench scores PASS only when this verifier passes
  // against the final observation.

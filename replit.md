...
2. **Org View** - Unified view combining outline editing and agenda into a single mode with 5 tabs: Outline, Today, Week, TODOs, Done. File filter bar at top allows toggling which org files are visible across all tabs. Outline tab: hierarchical bullet tree (Roam Research-style) with drag-and-drop, guide lines, collapsible children, draggable body paragraphs, backlinks, inline editing. Today/Week tabs: Emacs-style agenda with overdue indicators, quick-add, reschedule. TODOs/Done tabs: flat filtered lists. All tabs respect the file filter — no file-name section headers, items from all selected files appear in a single flat stream.

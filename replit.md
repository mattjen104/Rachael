...
- **Research Radar** (research-radar, program id=3): Expanded to 16 Reddit AI/LLM subreddits (LocalLLaMA, MachineLearning, OpenAI, ClaudeAI, Anthropic, LLMDevs, singularity, ollama, LangChain, StableDiffusion, comfyui, SelfHosted, OpenClaw, agi, ArtificialInteligence, artificial) + HN, GitHub trending, Lobsters, Lemmy, ArXiv. Fetches top comment per Reddit post. Two-stage Claude Sonnet pipeline: stage 1 filters for relevance to agent systems/local LLM/browser automation/voice/KB/OpenClaw, stage 2 synthesizes briefing. Hardened scraping: `retryFetch()` with backoff, 429 handling, rate limiting between Reddit requests (600ms/post, 1s/batch). Timeout raised to 600s.

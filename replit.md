...
10. **Inline Capture via Minibuffer** - Selecting `org-capture` from the minibuffer enters a multi-step inline capture wizard (like Emacs `org-capture` templates). Steps through: template selection (`[t] todo`, `[n] note`, `[l] link`) → title → file picker → scheduled date → tags. Each step is a single prompt in the minibuffer with completions above. Escape steps backward. No modal required.
11. **Clipboard Refile** - Each clipboard item has a `[r]` refile button that opens the minibuffer capture flow pre-filled with the item's content. After successful capture, the clipboard item is archived. Bridges the quick-dump clipboard with structured org-mode filing.

...
- **Research Radar** (research-radar, program id=3): Aggregates 16 Reddit AI/LLM subreddits (LocalLLaMA, MachineLearning, OpenAI, ClaudeAI, Anthropic, LLMDevs, singularity, ollama, LangChain, StableDiffusion, comfyui, SelfHosted, OpenClaw, agi, ArtificialInteligence, artificial) + HN, GitHub trending, Lobsters, Lemmy, ArXiv. Reddit uses RSS feeds (`.rss` endpoint) since JSON API blocks server-side requests (403). Two-stage Claude Sonnet pipeline: stage 1 filters for relevance to agent systems/local LLM/browser automation/voice/KB/OpenClaw, stage 2 synthesizes briefing. Hardened scraping: `retryFetch()` with backoff, 429 handling, 500ms delay between Reddit RSS fetches. Timeout raised to 600s. Code stored in DB; update via `scripts/fix-radar-code.ts`.

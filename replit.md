...
2. **Org View** - Unified view with 5 tabs: Outline, Today, Week, TODOs, Done. **Outline tab** = single-document editor with a buffer tab bar (Emacs-style) to switch between org files. Shows file title header (from `#+TITLE:`) and full heading tree for the selected file. **Today tab** = daily page backed by `journal.org`. Shows date header, unified DailyInput (smart parsing: `t ` task, `>` nesting, `[[` backlinks, plain text = note), scheduled items, and daily log entries. Tasks cross-file to inbox.org; notes stay in journal. Captures and completions from any view auto-create references in the daily log. **Other agenda tabs** (Week/TODOs/Done) = cross-file flat lists showing ALL items from ALL files. `§ filename` links navigate to file outline. **Keyboard-driven**: `1-5` switch tabs, `[`/`]` cycle buffers, `j`/`k` cursor navigation with hl-line-style highlight, `?` which-key overlay. **Toggleable hints**: inline keybinding labels on tabs/buffer bar/footer, toggle via `M-x toggle-hints` or localStorage. Features: drag-and-drop, guide lines, collapsible children, draggable body paragraphs, backlinks, inline editing.
4. **Clipboard View** - History/filing view for items captured via Chrome extension or ambient paste. Each item shows filing buttons `[t]` (todo), `[n]` (note), `[l]` (link) to file into org. Content type detection (URL/gif/image/code) with metadata enrichment. Smart capture input removed — now lives on the Today tab as DailyInput.
5. **Org Capture Modal** - Quick task creation (keyboard shortcut `Alt+C`). Fields: title, target file, scheduled date, tags. Appends TODO to file's INBOX section. Also triggered by pasting outside any input field (auto-detects URL vs text, pre-fills template).
6. **Daily Page System** - `journal.org` auto-populates with daily headings (`* YYYY-MM-DD Day`). Captures create references under today's heading. Completing a task (TODO→DONE) adds a CLOSED timestamp and logs a reference in the daily page. DailyInput on Today tab writes directly to journal. Tasks also cross-file to inbox.org for outliner visibility.
